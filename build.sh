...
SAW2_REXGLUE_PERF_COUNTERS="${SAW2_REXGLUE_PERF_COUNTERS:-OFF}"
case "${SAW2_REXGLUE_PERF_COUNTERS^^}" in
  ON|OFF) ;;
  *) saw2_die "SAW2_REXGLUE_PERF_COUNTERS must be ON or OFF" ;;
esac
...
saw2_run cmake --preset "$SAW2_SDK_PRESET" -S "$SDK_SOURCE" \
  ...
  -DREXGLUE_ENABLE_PERF_COUNTERS:BOOL="$SAW2_REXGLUE_PERF_COUNTERS"
readonly SDK_BUILD_DIR="$SDK_SOURCE/out/build/$SAW2_SDK_PRESET"
saw2_run cmake --build "$SDK_BUILD_DIR" \
  --config "$SDK_CMAKE_CONFIG" \
  --target install --parallel "$JOBS"

# ReXGlue upstream installs the CLI only for Release. Saw II uses the CLI from
# generated/rexglue.cmake in the host configuration (normally RelWithDebInfo),
# so ensure that exact CLI target exists and teach the installed CMake package
# where to find it for the selected configuration.
saw2_info "ensuring ReXGlue CLI for $SDK_CMAKE_CONFIG"
saw2_run cmake --build "$SDK_BUILD_DIR" \
  --config "$SDK_CMAKE_CONFIG" \
  --target rexglue --parallel "$JOBS"

case "$SDK_CMAKE_CONFIG" in
  Debug) SDK_CLI_FILENAME="rexglued" ;;
  RelWithDebInfo) SDK_CLI_FILENAME="rexgluerd" ;;
  Release) SDK_CLI_FILENAME="rexglue" ;;
  *) saw2_die "unsupported ReXGlue CLI configuration: $SDK_CMAKE_CONFIG" ;;
esac

SDK_CLI_BUILD="$SDK_SOURCE/out/$SAW2_SDK_PRESET/$SDK_CMAKE_CONFIG/$SDK_CLI_FILENAME"
if [[ ! -x "$SDK_CLI_BUILD" ]]; then
  SDK_CLI_BUILD="$(find "$SDK_SOURCE/out/$SAW2_SDK_PRESET" -maxdepth 3 -type f -name "$SDK_CLI_FILENAME" -perm -u+x -print -quit 2>/dev/null || true)"
fi
[[ -n "$SDK_CLI_BUILD" && -x "$SDK_CLI_BUILD" ]] ||
  saw2_die "built ReXGlue CLI not found for $SDK_CMAKE_CONFIG ($SDK_CLI_FILENAME)"

mkdir -p "$SDK_PREFIX/bin"
saw2_run install -m 0755 "$SDK_CLI_BUILD" "$SDK_PREFIX/bin/rexglue"

REXGLUE_CLI="$SDK_PREFIX/bin/rexglue"

# Do not patch ReXGlue's installed CMake exports. Upstream installs the CLI
# target location only for Release, while Saw II is normally built as
# RelWithDebInfo. The host configure gets a temporary CMake project-include
# below that fixes only the imported executable target after rexglue is loaded.
readonly REXGLUE_CLI
[[ -x "$REXGLUE_CLI" ]] ||
  saw2_die "local ReXGlue CLI is missing: $REXGLUE_CLI"
saw2_info "ReXGlue CLI import: $SDK_CMAKE_CONFIG -> $REXGLUE_CLI"
...
declare -a codegen_command=(
  "$REXGLUE_CLI"
  --log-level info
  --log-file "$CODEGEN_LOG"
  codegen
)

if grep -Rqs --include='*.cpp' '__builtin_ia32_pause' "$ROOT_DIR/generated/default" 2>/dev/null; then
  saw2_info "legacy db16cyc host pause detected; forcing one codegen regeneration"
  codegen_command+=(--ignore-stamp)
else
  saw2_info "codegen: using dependency stamp"
fi
codegen_command+=("$SAW2_MANIFEST")
saw2_run "${codegen_command[@]}"

# generated/rexglue.cmake uses $<TARGET_FILE:rex::rexglue> for its codegen
# custom command. The installed SDK doesn't provide an imported CLI location
# for RelWithDebInfo, so defer a target-property fix until the end of the
# top-level CMake directory, after find_package(rexglue) has created the target.
CMAKE_FIX_DIR="$ROOT_DIR/out/cmake"
CMAKE_REXGLUE_CLI_FIX="$CMAKE_FIX_DIR/saw2-rexglue-cli-import.cmake"
mkdir -p "$CMAKE_FIX_DIR"

cat > "$CMAKE_REXGLUE_CLI_FIX" <<EOF
# Generated by Saw II build.sh. Kept under out/ and never committed.
function(_saw2_fix_rexglue_cli_import)
  if(NOT TARGET rex::rexglue)
    message(FATAL_ERROR "Saw II: rex::rexglue target was not created by the SDK")
  endif()

  foreach(_saw2_cfg DEBUG RELEASE RELWITHDEBINFO MINSIZEREL)
    set_property(
      TARGET rex::rexglue
      APPEND PROPERTY IMPORTED_CONFIGURATIONS \${_saw2_cfg})
    set_target_properties(
      rex::rexglue
      PROPERTIES IMPORTED_LOCATION_\${_saw2_cfg} "$REXGLUE_CLI")
  endforeach()
endfunction()

cmake_language(DEFER CALL _saw2_fix_rexglue_cli_import)
EOF

saw2_info "host CMake CLI fix: $CMAKE_REXGLUE_CLI_FIX"
...
declare -a configure_extra=(
  ...
  -DCMAKE_PROJECT_INCLUDE:FILEPATH="$CMAKE_REXGLUE_CLI_FIX"
)
